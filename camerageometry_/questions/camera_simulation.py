...
def calculate_camera_matrix(tx, ty, tz, alpha, beta, gamma, fx, fy, skew, u, v):
    ...
    initial_intrinsic_matrix_to_replace[0,0] = fx
    initial_intrinsic_matrix_to_replace[0,1] = skew
    initial_intrinsic_matrix_to_replace[0,2] = u
    initial_intrinsic_matrix_to_replace[1,0] = 0
    initial_intrinsic_matrix_to_replace[1,1] = fy
    initial_intrinsic_matrix_to_replace[1,2] = v
    initial_intrinsic_matrix_to_replace[2,0] = 0
    initial_intrinsic_matrix_to_replace[2,1] = 0
    initial_intrinsic_matrix_to_replace[2,2] = 1

    initial_extrinsic_matrix_to_replace[0,0] = np.cos(beta) * np.cos(gamma)
    initial_extrinsic_matrix_to_replace[0,1] = np.cos(alpha) * np.sin(beta) * np.cos(gamma) - np.cos(alpha) * np.sin(gamma)
    initial_extrinsic_matrix_to_replace[0,2] = np.cos(alpha) * np.sin(beta) * np.cos(gamma) + np.sin(alpha) * np.sin(gamma)
    initial_extrinsic_matrix_to_replace[0,3] = tx
    initial_extrinsic_matrix_to_replace[1,0] = np.cos(beta) * np.sin(gamma)
    initial_extrinsic_matrix_to_replace[1,1] = np.sin(alpha) * np.sin(beta) * np.sin(gamma) + np.cos(alpha) * np.cos(gamma)
    initial_extrinsic_matrix_to_replace[1,2] = np.cos(alpha) * np.sin(beta) * np.sin(gamma) - np.sin(alpha) * np.cos(gamma)
    initial_extrinsic_matrix_to_replace[1,3] = ty
    initial_extrinsic_matrix_to_replace[2,0] = -np.sin(beta)
    initial_extrinsic_matrix_to_replace[2,1] = np.sin(alpha) * np.cos(beta)
    initial_extrinsic_matrix_to_replace[2,2] = np.cos(alpha) * np.cos(beta)
    initial_extrinsic_matrix_to_replace[2,3] = tz

    initial_camera_matrix_to_replace = np.matmul(initial_intrinsic_matrix_to_replace, initial_extrinsic_matrix_to_replace)
